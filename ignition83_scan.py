...
SKIP_DIRS = {
    ".git", ".resources", ".idea", ".vs", ".vscode", "node_modules",
    "dist", "build", "__pycache__", ".pytest_cache", ".mypy_cache",
}

TEXT_EXTENSIONS = {
    ".py", ".txt", ".json", ".xml", ".yaml", ".yml", ".properties",
    ".md", ".sql", ".js", ".ts", ".tsx", ".jsx",
}

SEVERITY_ORDER = {"RED": 0, "ORANGE": 1, "YELLOW": 2, "GREEN": 3}

REQUIRED_RULE_FIELDS = {
    "rule_id", "severity", "category", "pattern", "description", "reason",
    "recommendation", "test_procedure", "affected_versions", "fixed_version",
...
def load_rules(path: Path) -> List[Rule]:
    try:
        with path.open("r", newline="", encoding="utf-8-sig") as f:
            ...
            if missing:
                raise ValueError(
                    "Rules CSV is missing required columns: " + ", ".join(sorted(missing))
                )
            ...
            for row_number, row in enumerate(reader, start=2):
                ...
                if severity not in SEVERITY_ORDER:
                    raise ValueError(
                        f"Invalid severity '{severity}' for rule {rule_id} on row {row_number}."
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid regex for rule {rule_id} on row {row_number}: {e}"
                    )
...
def sort_findings(findings: List[Finding]) -> List[Finding]:
    return sorted(
        findings,
        key=lambda f: (
            SEVERITY_ORDER.get(f.severity, 99), f.project, f.file, f.line, f.rule_id,
        ),
    )
...
def write_findings_csv(findings: List[Finding], filename: Path) -> None:
    ...
    with filename.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Severity", "Rule ID", "Project", "File", "Line", "Code", "Issue",
            "Recommendation",
        ])
        for finding in findings:
            writer.writerow([
                finding.severity, finding.rule_id, finding.project, finding.file,
                finding.line, finding.code, finding.message, finding.recommendation,
            ])
...
def write_rule_summary_csv(
    ...
    for finding in findings:
        ...
        projects_by_rule.setdefault(finding.rule_id, set()).add(finding.project)

    ordered_rules = sorted(
        {rule.rule_id: rule for rule in rules}.values(),
        key=lambda r: (SEVERITY_ORDER.get(r.severity, 99), r.category, r.rule_id),
    )

    with filename.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Rule ID", "Severity", "Category", "Enabled", "Occurrences",
            "Projects Affected", "Project Count", "Description", "Reason",
        ...
        for rule in ordered_rules:
            ...
            writer.writerow([
                rule.rule_id, rule.severity, rule.category,
                "Yes" if rule.enabled else "No", counts.get(rule.rule_id, 0),
                "; ".join(projects), len(projects), rule.description, rule.reason,
                rule.recommendation, rule.test_procedure, rule.affected_versions,
                rule.fixed_version, rule.reference, rule.status, rule.notes, rule.pattern,
            ])


def main():
    parser = argparse.ArgumentParser(
        description="Scan an Ignition installation for 8.1 -> 8.3.8 compatibility hazards."
    )
    parser.add_argument(
        "root",
        nargs="?",
        default=DEFAULT_WINDOWS_ROOT,
        help=f"Ignition installation directory (default: {DEFAULT_WINDOWS_ROOT})",
    )
    parser.add_argument(
        "--rules",
        default="ignition83_rules.csv",
        help="CSV rules file (default: ignition83_rules.csv)",
    )
    parser.add_argument(
        "--reports",
        default="reports",
    ...
    try:
        findings = sort_findings(scan_repository(projects_root, rules))
    ...
    print(
        f"Complete in {elapsed:.2f}s | {len(findings)} findings | "
